public class ArmController extends Thread{
    ...
    public ArmController(Square square, UserInterface UI, ArmDirection dir) {
        this.square = square;
        this.UI = UI;
        this.direction = dir;
    }
    ...
    public void move() throws ArmOutOfBoundException {
        ...
        switch(direction){
            case X_DIRECTION_RIGHT: position.incrPos(Position.PositionDirection.X_POS);
                break;
            case X_DIRECTION_LEFT: position.decrPos(Position.PositionDirection.X_POS);
                break;
            case Y_DIRECTION_DOWN: position.incrPos(Position.PositionDirection.Y_POS);
                break;
            case Y_DIRECTION_UP: position.decrPos(Position.PositionDirection.Y_POS);
                break;
            default: throw new IllegalArgumentException("Unkown direction argument");
        }

        // check if new position possible
        if(!square.isBetweenBorders(position)) {
